...
namespace cycfi
{
	...
	template <typename T>
	using remove_cvref_t = typename remove_cv_ref<T>::type;

	template <typename T, typename... Rest>
	struct is_arithmetic
	{
		constexpr static bool value = std::is_arithmetic<T>::value && is_arithmetic<Rest...>::value;
	};
	...
	inline T& clamp_max(T& val, const U& max)
	{
		if (val > max)
			val = max;
		return val;
	...
	template <typename T, typename U>
	inline T& clamp_min(T& val, const U & min)
	{
		if (val < min)
			val = min;
		return val;
	...
	inline T& clamp(T& val, const U & min, const V & max)
	{
		assert(min <= max);
		clamp_min(val, min);
		clamp_max(val, max);
