...
{
	basic_menu button_menu(std::string text, menu_position pos, color body_color)
	{
		auto icon =
	...
	basic_menu button_menu(menu_position pos, color body_color)
	{
		auto icon =
				(pos == menu_position::bottom_right || pos == menu_position::bottom_left)
						?
	...
	view_limits menu_item_spacer_element::limits(basic_context const& /* ctx */) const
	{
		...
		return { 0, height, full_extent<view_limits::coordinate_type>, height };
	}
		...
		auto menu_btn = text_button<basic_menu>(
				margin(
						...
						htile(
								...
								align_right(
										left_margin(12, icon(icons::down_dir, 1.0))
										)
								)
						)
				);
	...
	{
		auto r = build_selection_menu_label(std::move(init));

		// todo: item horizontal size should be the same as the label(the menu button)
		if (items.size())
		{
			...
			for (std::size_t i = 0; i != items.size(); ++i)
			{
				...
				if (label)
				{
					e->on_click = [btn_text = r.second, on_select, label, where = i]()
					{
						const auto& text = label->get_text();
						btn_text->set_text(text);
						on_select(text, where);
					};
				}
	        ...
	        display_shortcut(key_code k, int mod)
	{
		...
		else
		{
			switch (k)
			{
				...
				case key_code::_9:
					key_ += std::to_string('0' + (int(k) - int(key_code::_0)));
					break;
				...
				case key_code::y: case key_code::z:
					key_ += char('A' + (int(k) - int(key_code::a)));
					break;
				...
				case key_code::kp_9:
					key_ += '0' + std::to_string((int(k) - int(key_code::kp_0)));
					break;
