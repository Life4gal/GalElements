...
namespace cycfi::elements
{
	...
	rect canvas::clip_extent() const
	{
		...
		return {
				static_cast<rect::coordinate_type>(x1),
				static_cast<rect::coordinate_type>(y1),
				static_cast<rect::coordinate_type>(x2),
				static_cast<rect::coordinate_type>(y2)
		};
	}
	...
	void canvas::fill_style(color c)
	{
		...
		if (_state.pattern_set == canvas_state::pattern_state::fill_set)
			_state.pattern_set = canvas_state::pattern_state::none_set;
	}
	...
	void canvas::stroke_style(color c)
	{
		...
		if (_state.pattern_set == canvas_state::pattern_state::stroke_set)
			_state.pattern_set = canvas_state::pattern_state::none_set;
	}
	...
	{
		if (font_.font_handle)
			cairo_set_font_face(&_context, font_.font_handle);
	}
	...
	namespace
	{
		point get_text_start(cairo_t& _context, point p, canvas::text_alignment align, const char * utf8)
		{
			...
			switch (canvas::text_alignment::vertical_mask & align)
			{
				...
				case canvas::text_alignment::bottom:
					...
					break;
				case canvas::text_alignment::baseline:
					[[fallthrough]];
				default:
					break;
			}
	...
	canvas::canvas_state::canvas_state() : align(text_alignment::left | text_alignment::baseline), pattern_set(pattern_state::none_set) {}
}
