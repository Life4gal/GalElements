...
namespace cycfi::elements
{
	struct host_view
	{
		host_view() : im_context(gtk_im_context_simple_new()) {}
	...
	namespace
	{
		...
		gboolean on_draw(GtkWidget* /* widget */, cairo_t* cr, gpointer user_data)
		{
			...
			view.draw(
					cr,
					{
							static_cast<rect::coordinate_type>(left),
							static_cast<rect::coordinate_type>(top),
							static_cast<rect::coordinate_type>(right),
							static_cast<rect::coordinate_type>(bottom)
					}
            );
		...
		bool get_mouse(Event* event, mouse_button& btn, host_view* view)
		{
			...
			btn.pos = {
					static_cast<typename decltype(btn.pos)::coordinate_type>(event->x),
					static_cast<typename decltype(btn.pos)::coordinate_type>(event->y)
			};
			return true;
		}
		...
		gboolean on_motion(GtkWidget* /* widget */, GdkEventMotion* event, gpointer user_data)
		{
			...
			if (get_mouse(event, btn, view))
			{
				...
				{
					btn.down = true;
					btn.state = mouse_button::what::left;
				}
				else if (event->state & GDK_BUTTON2_MASK)
				{
					btn.down = true;
					btn.state = mouse_button::what::middle;
				}
				else if (event->state & GDK_BUTTON3_MASK)
				{
					btn.down = true;
					btn.state = mouse_button::what::right;
				}
				else if (event->state & GDK_BUTTON4_MASK)
				{
					btn.down = true;
					btn.state = mouse_button::what::x1;
				}
				else if (event->state & GDK_BUTTON5_MASK)
				{
					btn.down = true;
					btn.state = mouse_button::what::x2;
				}
				else
				{
					btn.down = false;
		...
		gboolean on_scroll(GtkWidget* /* widget */, GdkEventScroll* event, gpointer user_data)
		{
			...
			base_view.scroll(
					{ dx, dy },
					{ static_cast<point::coordinate_type>(event->x), static_cast<point::coordinate_type>(event->y) }
            );
			return true;
		}
	}
...
//									  | GDK_SMOOTH_SCROLL_MASK
	...
	base_view::base_view(extent /* size */) : base_view(new host_view)
	{
		// $$$ FIXME: Implement Me $$$
	...
	base_view::base_view(host_window_handle h) : base_view(new host_view)
	{
		auto make_view =
				[this, h]()
		{
	...
	void base_view::resize(elements::extent new_size)
	{
		// $$$ Wrong: don't resize the window!!! $$$
		gtk_window_resize(GTK_WINDOW(_view->widget), new_size.width, new_size.height);
	}

	float base_view::hdpi_scale() const
	{
		// ide say this function can be made static
		unused(this);
	...
	void base_view::refresh()
	{
		auto x = gtk_widget_get_allocated_width(_view->widget);
		auto y = gtk_widget_get_allocated_height(_view->widget);
		refresh({ 0, 0, static_cast<rect::coordinate_type>(x), static_cast<rect::coordinate_type>(y) });
	}
